...
from .splat import SplitAttnConv2d




__all__ = ['ResNest', 'resnest50', 'resnest101', 'resnest200']
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
                 ...
                 avd=True, avd_first=False, dilation=1, is_first=False,
...
def resnest50(pretrained=False, **kwargs):
    ...
    if pretrained:
        print('Initialize with pre-trained ResNest50')
        from collections import OrderedDict
        state_dict = model.state_dict()
        pretrained_state_dict = model_zoo.load_url(model_urls['resnest50'])
...
def resnest101(pretrained=False, **kwargs):
    ...
    model = ResNest(Bottleneck, [3, 4, 23, 3],
                   ...
                   avd=True, avd_first=False, **kwargs)
    if pretrained:
        print('Initialize with pre-trained ResNest101')
        from collections import OrderedDict
        state_dict = model.state_dict()
        pretrained_state_dict = model_zoo.load_url(model_urls['resnest101'])
        for k, v in pretrained_state_dict.items():
            if k not in state_dict:
                continue
            state_dict[k] = v
...
def resnest200(pretrained=False, **kwargs):
    ...
    model = ResNest(Bottleneck, [3, 24, 36, 3],
                   ...
                   avd=True, avd_first=False, **kwargs)
    if pretrained:
        print('Initialize with pre-trained ResNest200')
        from collections import OrderedDict
        state_dict = model.state_dict()
        pretrained_state_dict = model_zoo.load_url(model_urls['resnest200'])
        for k, v in pretrained_state_dict.items():
            if k not in state_dict:
                continue
            state_dict[k] = v
