...
class HRfineNet(nn.Module):
    def __init__(self, lateral_channel, out_shape, num_class): #later_channel = 256
        super(HRfineNet, self).__init__()
        cascade = []
        num_cascade = 4
    ...
    def forward(self, x):
        fms = []
        for i in range(4):
            fms.append(self.cascade[i](x[i]))
        out = torch.cat(fms, dim=1)
