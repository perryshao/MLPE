...
class ResNet(nn.Module): #三要素1：继承nn.Module
    ...
    def _make_layer(self, block, planes, blocks, stride=1):  #以一个下划线开头命名：表示它是一个受保护的方法，在本类或者子类中访问
        downsample = None
        ...
        # 继承自Modu类。是一个顺序容器，在模型构建时，将组件按照一定的顺序保护起来，这些组件只有顺序关系，没有链接关系，组件间的链接关系在forward()中实现
...
def resnet50(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], **kwargs) #参数有模块，Bottleneck
    if pretrained:
...
def resnet152(pretrained=False, **kwargs):
    ...
    if pretrained:
        print('Initialize with pre-trained ResNet152')
        from collections import OrderedDict
        state_dict = model.state_dict()
